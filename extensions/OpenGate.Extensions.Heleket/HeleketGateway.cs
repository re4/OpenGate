...
public class HeleketGateway : IPaymentGateway
{
    ...
    public string? Description => "Heleket cryptocurrency payment gateway integration";
    ...
    public async Task<PaymentResult> CreatePaymentAsync(PaymentRequest request)
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                {
                    Success = false,
                    ErrorMessage = $"Heleket API error ({(int)response.StatusCode}): {content}"
                };
            }
    ...
    public Task<PaymentResult> RefundAsync(string transactionId, decimal amount)
    {
        return Task.FromResult(new PaymentResult
        {
            Success = false,
            TransactionId = transactionId,
            ErrorMessage = "Refunds are not supported for cryptocurrency payments via Heleket."
        });
    }
